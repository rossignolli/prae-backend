...
equipamentsRouter.get('/home', async (request, response) => {
    ...
    equipaments.map(equipament => {
        const isEquipamentExpiredAndIsNotBeingMonitored =
            isPast(equipament.dateOfExpiration) && equipament.monitor !== false;

        const isOperatinal = isFuture(equipament.dateOfExpiration);
        const isEquipamentExpiring =
            differenceInDays(equipament.dateOfExpiration, Date.now()) <= 10 &&
            isFuture(equipament.dateOfExpiration);
        ...
        if (isEquipamentExpiredAndIsNotBeingMonitored) {
    ...
    const fixTotalOperational =
        operationalEquipaments.length - expiringEquipaments.length;
    ...
    const operationalPercentagen = fixTotalOperational / equipaments.length;
    ...
    try {
        ...
        const docDef: TDocumentDefinitions = {
            ...
            content: [
                ...
                {
                    text: 'Top 5 equipamentos mais problemáticos',
                    alignment: 'center',
                    margin: [0, 10, 0, 20],
                },
                {
