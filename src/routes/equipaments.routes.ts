/* eslint-disable import/extensions */
import { Router } from 'express';
import Equipament from '../models/Equipament';
import { getRepository } from 'typeorm';
import CreateEquipamentService from '../services/CreateEquipamentService';
import UpdateEquipamentService from '../services/UpdateEquipamentService';
import ensureAuthenticated from '../middlewares/ensureAuthenticated';
import { differenceInDays, isPast, isFuture } from 'date-fns';
import QRCode from 'qrcode';
import multer from 'multer';
import uploadConfig from '../config/upload';
import Preventive from '../models/Preventives';
import Brand from '../models/Brand';
import User from '../models/User';
import { getManager } from 'typeorm';
import JobExecution from '../models/JobExecution';
import { TDocumentDefinitions } from 'pdfmake/interfaces';
import PDFPrinter from 'pdfmake';
import Images from '../models/Images';
import aws from 'aws-sdk';
const equipamentsRouter = Router();
//@ts-ignore
const upload = multer(uploadConfig);

interface AWSFiles {
    originalname: string;
    location: string;
}

equipamentsRouter.use(ensureAuthenticated);

equipamentsRouter.get('/details/:id', async (request, response) => {
    const { id } = request.params;
    const equipamentsRepository = getRepository(Equipament);
    const preventiveRepository = getRepository(Preventive);

    const equipament = await equipamentsRepository.findOneOrFail(id, {
        relations: ['images'],
    });

    const lastCorrective = await preventiveRepository.findOne({
        where: { equipament_id: id, isCorrective: true },
    });

    const findAllMaintenances = await preventiveRepository.find({
        where: { equipament_id: id },
        order: { created_at: 'DESC' },
    });

    const lastCorrectivePreventiveDate = lastCorrective?.created_at;

    if (!lastCorrectivePreventiveDate) {
        const equipamentWithoutCorrective = {
            ...equipament,
            daysWithoutCorrective: 0,
            maintenanceTotal: findAllMaintenances.length,
        };
        return response.json(equipamentWithoutCorrective);
    }

    if (lastCorrectivePreventiveDate) {
        const daysWithoutCorrective = differenceInDays(
            new Date(),
            lastCorrectivePreventiveDate,
        );

        const equipamentWithCorrective = {
            ...equipament,
            daysWithoutCorrective: daysWithoutCorrective,
            maintenanceTotal: findAllMaintenances.length,
        };

        return response.json(equipamentWithCorrective);
    }
});

equipamentsRouter.delete('/:id', async (request, response) => {
    const { id } = request.params;

    try {
        const equipamentsRepository = getRepository(Equipament);
        const preventive = getRepository(Preventive);
        const jobsRepository = getRepository(JobExecution);
        const imagesRepository = getRepository(Images);

        const allPreventives = await preventive.find({ equipament_id: id });

        allPreventives.map(async preventive => {
            await jobsRepository.delete({
                preventive_id: preventive.id,
            });
        });

        await preventive.delete({ equipament_id: id });

        const allImages = await imagesRepository.find({
            where: { equipament_id: id },
        });

        allImages.map(async image => {
            const imagetoDelete = image.path.split('/').pop();

            if (imagetoDelete) {
                const s3 = new aws.S3();
                const params = {
                    Bucket: 'static-prae',
                    Key: imagetoDelete,
                };

                try {
                    await s3.headObject(params).promise();

                    try {
                        await s3.deleteObject(params).promise();
                    } catch (err) {
                        console.log(
                            'ERROR in file Deleting : ' + JSON.stringify(err),
                        );
                    }
                } catch (err) {
                    console.log('File not Found');
                }
            }
        });

        await imagesRepository.delete({ equipament_id: id });
        await equipamentsRepository.delete({ id });

        return response.status(200).json({ ok: 'true' });
    } catch (err) {
        //@ts-ignore
        return response.status(err.StatusCode).json({ error: err.message });
    }
});

equipamentsRouter.get('/', async (request, response) => {
    const equipamentsRepository = getRepository(Equipament);

    const equipaments = await equipamentsRepository.find({
        relations: ['images'],
        order: { dateOfExpiration: 'ASC' },
    });

    const ExpirationClassification: any[] = [];

    equipaments.map(equipament => {
        const isEquipamentExpired = () => {
            if (equipament.monitor === true) {
                const daysToExpiration = differenceInDays(
                    equipament.dateOfExpiration,
                    new Date(),
                );
                if (daysToExpiration <= 0) {
                    return true;
                }
            }
        };

        const isOperatinal = () => {
            if (equipament.monitor === true) {
                const daysToExpiration = differenceInDays(
                    equipament.dateOfExpiration,
                    new Date(),
                );
                if (daysToExpiration > 10) {
                    return true;
                }
            }
        };

        const isEquipamentExpiring = () => {
            if (equipament.monitor === true) {
                const daysToExpiration = differenceInDays(
                    equipament.dateOfExpiration,
                    new Date(),
                );
                if (daysToExpiration < 10 && daysToExpiration > 0) {
                    return true;
                }
            }
            ``;
        };

        const isEquipamentNotBeingMonitored = !equipament.dateOfExpiration;

        if (isEquipamentExpiring()) {
            ExpirationClassification.push({
                ...equipament,
                status: 'expiring',
            });
        }

        if (isEquipamentExpired()) {
            ExpirationClassification.push({ ...equipament, status: 'expired' });
        }

        if (isOperatinal()) {
            ExpirationClassification.push({
                ...equipament,
                status: 'operational',
            });
        }

        if (isEquipamentNotBeingMonitored) {
            ExpirationClassification.push({
                ...equipament,
                status: 'noMonitor',
            });
        }
    });

    return response.json(ExpirationClassification);
});

equipamentsRouter.get('/home', async (request, response) => {
    const equipamentsRepository = getRepository(Equipament);
    const preventiveRepository = getRepository(Preventive);
    const brandRepository = getRepository(Brand);
    const userRepository = getRepository(User);

    const equipaments = await equipamentsRepository.find({
        order: { dateLastStopMonitor: 'DESC' },
    });

    const preventive = await preventiveRepository.find();
    const brand = await brandRepository.find();
    const user = await userRepository.find();

    const expiredEquipaments: any = [];
    const expiringEquipaments: any = [];
    const notBeingMonitored: any = [];
    const operationalEquipaments: any = [];

    equipaments.map(equipament => {
        const isEquipamentExpired = () => {
            if (equipament.monitor === true) {
                const daysToExpiration = differenceInDays(
                    equipament.dateOfExpiration,
                    new Date(),
                );
                if (daysToExpiration <= 0) {
                    return true;
                }
            }
        };

        const isOperatinal = () => {
            if (equipament.monitor === true) {
                const daysToExpiration = differenceInDays(
                    equipament.dateOfExpiration,
                    new Date(),
                );
                if (daysToExpiration > 10) {
                    return true;
                }
            }
        };

        const isEquipamentExpiring = () => {
            if (equipament.monitor === true) {
                const daysToExpiration = differenceInDays(
                    equipament.dateOfExpiration,
                    new Date(),
                );
                if (daysToExpiration < 10 && daysToExpiration > 0) {
                    return true;
                }
            }
        };

        const isEquipamentNotBeingMonitored = !equipament.dateOfExpiration;

        if (isEquipamentExpiring()) {
            expiringEquipaments.push({ ...equipament, status: 'expiring' });
        }

        if (isEquipamentExpired()) {
            expiredEquipaments.push({ ...equipament, status: 'expired' });
        }

        if (isOperatinal()) {
            operationalEquipaments.push({
                ...equipament,
                status: 'operational',
            });
        }

        if (isEquipamentNotBeingMonitored) {
            notBeingMonitored.push(equipament);
        }
    });

    const expiredPercentage = expiredEquipaments.length / equipaments.length;
    const notBeingMonitoredPercentage =
        notBeingMonitored.length / equipaments.length;

    const expiringPercentagen = expiringEquipaments.length / equipaments.length;
    console.log(notBeingMonitored.length);

    const operationalPercentagen =
        operationalEquipaments.length / equipaments.length;

    const homeData = {
        equipamentsTotal: equipaments.length,
        preventiveTotal: preventive.length,
        brandTotal: brand.length,
        userTotal: user.length,
        percentage: {
            operational: parseFloat(operationalPercentagen.toFixed(2)) * 100,
            expired: parseFloat(expiredPercentage.toFixed(2)) * 100,
            expiring: parseFloat(expiringPercentagen.toFixed(2)) * 100,
            notBeingMonitored:
                parseFloat(notBeingMonitoredPercentage.toFixed(2)) * 100,
        },
        equipaments: [...expiredEquipaments, ...expiringEquipaments],
    };

    const entityManager = getManager();

    return response.json(homeData);
});

equipamentsRouter.post(
    '/',
    upload.array('images'),
    async (request, response) => {
        const { data } = request.body;

        const requestImages = (request.files as unknown) as AWSFiles[];

        console.log(requestImages);

        const images = requestImages.map(image => {
            return { path: image.location };
        });

        const parsedEquipament = JSON.parse(data);

        parsedEquipament.images = images;

        const createEquipamentService = new CreateEquipamentService();

        const equipament = await createEquipamentService.execute(
            parsedEquipament,
        );

        return response.json(equipament);
    },
);

equipamentsRouter.put(
    '/:id',
    upload.array('newImages'),
    async (request, response) => {
        const { data } = request.body;
        const { id } = request.params;

        const requestImages = (request.files as unknown) as AWSFiles[];

        const images = requestImages.map(image => {
            return { path: image.location };
        });

        const parsedEquipament = JSON.parse(data);

        parsedEquipament.images = images;

        const updateEquipamentService = new UpdateEquipamentService();

        const equipament = await updateEquipamentService.execute({
            id,
            name: parsedEquipament.name,
            description: parsedEquipament.description,
            brand_id: parsedEquipament.brand,
            category_id: parsedEquipament.category,
            date: parsedEquipament.newExpiratiaonDate,
            images: parsedEquipament.images,
            deletedImages: parsedEquipament.deletedImagens,
        });

        return response.json(equipament);
    },
);

equipamentsRouter.get('/report', async (request, response) => {
    try {
        const equipamentsRepository = getRepository(Equipament);
        const equipaments = await equipamentsRepository.find();

        const body: any = [];
        const body2: any = [];

        equipaments.forEach((equipament, i) => {
            const rows = new Array();
            rows.push({
                text: `${i + 1}`,
                alignment: 'center',
                bold: true,
                margin: [0, 5, 0, 5],
            });
            rows.push({
                text: equipament.name,
                alignment: 'left',
                margin: [0, 5, 0, 5],
            });
            rows.push({
                text: equipament.brand.name,
                alignment: 'left',
                margin: [0, 5, 0, 5],
            });
            rows.push({
                text: equipament.technician.name,
                alignment: 'left',
                margin: [0, 5, 0, 5],
            });

            body.push(rows);
        });

        equipaments.forEach((equipament, i) => {
            const rows = new Array();
            rows.push({
                text: `${i + 1}`,
                alignment: 'center',
                bold: true,
                margin: [0, 5, 0, 5],
            });
            rows.push({
                text: equipament.name,
                alignment: 'center',
                margin: [0, 5, 0, 5],
            });
            rows.push({
                text: equipament.brand.name,
                alignment: 'center',
                margin: [0, 5, 0, 5],
            });
            rows.push({
                text: equipament.technician.name,
                alignment: 'center',
                margin: [0, 5, 0, 5],
            });

            body2.push(rows);
        });

        const fonts = {
            Helvetica: {
                normal: 'Helvetica',
                bold: 'Helvetica-Bold',
                italics: 'Helvetica-Oblique',
                bolditalics: 'Helvetica-BoldOblique',
            },
        };

        const printer = new PDFPrinter(fonts);
        const dateOptions = {
            weekday: 'long',
            year: 'numeric',
            month: 'long',
            day: 'numeric',
        };
        const today = new Date();

        const docDef: TDocumentDefinitions = {
            defaultStyle: { font: 'Helvetica' },
            content: [
                {
                    image: 'Praelogo',
                    width: 150,
                    height: 42,
                    alignment: 'center',
                },
                {
                    text: `Relatório de Equipamentos  `,
                    alignment: 'center',
                    style: 'header',
                    margin: [0, 20, 0, 0],
                },
                {
                    text: `${today.toLocaleString('pt-BR')}`,
                    alignment: 'center',
                    style: 'subtitle',
                    margin: [0, 5, 0, 0],
                },
                {
                    text: 'Listagem geral de equipamentos cadastrados.',
                    alignment: 'center',
                    margin: [0, 10, 0, 20],
                },
                {
                    style: 'tableExample',
                    table: {
                        widths: [100, '*', '*', '*'],
                        body: [
                            [
                                {
                                    text: 'ID',
                                    alignment: 'center',
                                    bold: true,
                                    margin: [0, 5, 0, 5],
                                },
                                {
                                    text: 'Equipamento',
                                    alignment: 'center',
                                    bold: true,
                                    margin: [0, 5, 0, 5],
                                },
                                {
                                    text: 'Marca',
                                    alignment: 'center',
                                    bold: true,
                                    margin: [0, 5, 0, 5],
                                },
                                {
                                    text: 'Técnico',
                                    alignment: 'center',
                                    bold: true,
                                    margin: [0, 5, 0, 5],
                                },
                            ],
                            ...body,
                        ],
                    },
                },
            ],
            styles: {
                header: {
                    fontSize: 18,
                    bold: true,
                },
                subtitle: {
                    fontSize: 16,
                    bold: true,
                    color: 'gray',
                },
            },
            images: {
                Praelogo:
                    'data:image/png;base64,',
            },
        };

        const pdfDoc = printer.createPdfKitDocument(docDef);

        const chunks: any = [];

        pdfDoc.on('data', chunk => {
            chunks.push(chunk);
        });
        pdfDoc.on('end', () => {
            const result = Buffer.concat(chunks);
            return response.end(result);
        });

        // close the stream
        pdfDoc.end();
    } catch (err) {
        return response.status(400).json({ ok: true });
    }
});

equipamentsRouter.get('/report/:id', async (request, response) => {
    try {
        const { id } = request.params;
        const equipamentsRepository = getRepository(Equipament);
        const preventiveRepository = getRepository(Preventive);

        const equipament = await equipamentsRepository.findOne({
            where: { id },
        });

        const preventives = await preventiveRepository.find({
            where: { equipament_id: id },
        });

        const body: any = [];
        const body2: any = [];

        const rows = new Array();
        rows.push({
            text: `${equipament?.id}`,
            alignment: 'center',
            bold: true,
            margin: [0, 5, 0, 5],
        });
        rows.push({
            text: equipament?.name,
            alignment: 'center',
            margin: [0, 5, 0, 5],
        });
        rows.push({
            text: equipament?.technician.name,
            alignment: 'center',
            margin: [0, 5, 0, 5],
        });
        rows.push({
            text: equipament?.category.name,
            alignment: 'center',
            margin: [0, 5, 0, 5],
        });

        body.push(rows);

        preventives.forEach((preventive, i) => {
            const rows = new Array();
            rows.push({
                text: `${i + 1}`,
                alignment: 'center',
                bold: true,
                margin: [0, 5, 0, 5],
            });
            rows.push({
                text: preventive.created_at.toLocaleString('pt-BR'),
                alignment: 'center',
                margin: [0, 5, 0, 5],
            });

            rows.push({
                text: `${preventive.isCorrective ? 'Corretiva' : 'Preventiva'}`,
                alignment: 'center',
                margin: [0, 5, 0, 5],
            });

            rows.push({
                text: preventive.technician.name,
                alignment: 'center',
                margin: [0, 5, 0, 5],
            });

            body2.push(rows);
        });

        const fonts = {
            Helvetica: {
                normal: 'Helvetica',
                bold: 'Helvetica-Bold',
                italics: 'Helvetica-Oblique',
                bolditalics: 'Helvetica-BoldOblique',
            },
        };

        const printer = new PDFPrinter(fonts);
        const dateOptions = {
            weekday: 'long',
            year: 'numeric',
            month: 'long',
            day: 'numeric',
        };
        const today = new Date();

        const docDef: TDocumentDefinitions = {
            defaultStyle: { font: 'Helvetica' },
            content: [
                {
                    image: 'Praelogo',
                    width: 150,
                    height: 42,
                    alignment: 'center',
                },
                {
                    text: `Relatório de Equipamento  `,
                    alignment: 'center',
                    style: 'header',
                    margin: [0, 20, 0, 0],
                },
                {
                    text: `${today.toLocaleString('pt-BR')}`,
                    alignment: 'center',
                    style: 'subtitle',
                    margin: [0, 5, 0, 0],
                },
                {
                    text: 'Detalhes sobre o equipamento:',
                    alignment: 'center',
                    margin: [0, 10, 0, 10],
                },
                {
                    text: `${equipament?.name} `,
                    alignment: 'center',
                    style: 'titlePurple',
                    fontSize: 18,
                    bold: true,
                    margin: [0, 5, 0, 10],
                },
                {
                    style: 'tableExample',
                    table: {
                        widths: [100, '*', '*', '*'],
                        body: [
                            [
                                {
                                    text: 'ID',
                                    alignment: 'center',
                                    bold: true,
                                    margin: [0, 5, 0, 5],
                                },
                                {
                                    text: 'Equipamento',
                                    alignment: 'center',
                                    bold: true,
                                    margin: [0, 5, 0, 5],
                                },
                                {
                                    text: 'Marca',
                                    alignment: 'center',
                                    bold: true,
                                    margin: [0, 5, 0, 5],
                                },
                                {
                                    text: 'Técnico',
                                    alignment: 'center',
                                    bold: true,
                                    margin: [0, 5, 0, 5],
                                },
                            ],
                            ...body,
                        ],
                    },
                },
                {
                    text:
                        'Ações de manutenção ja executadas nesse equipamento:',
                    alignment: 'center',
                    margin: [0, 10, 0, 10],
                },
                {
                    style: 'tableExample',
                    table: {
                        widths: [100, '*', '*', '*'],
                        body: [
                            [
                                {
                                    text: 'ID',
                                    alignment: 'center',
                                    bold: true,
                                    margin: [0, 5, 0, 5],
                                },
                                {
                                    text: 'Data de execução',
                                    alignment: 'center',
                                    bold: true,
                                    margin: [0, 5, 0, 5],
                                },
                                {
                                    text: 'Tipo',
                                    alignment: 'center',
                                    bold: true,
                                    margin: [0, 5, 0, 5],
                                },
                                {
                                    text: 'Técnico',
                                    alignment: 'center',
                                    bold: true,
                                    margin: [0, 5, 0, 5],
                                },
                            ],
                            ...body2,
                        ],
                    },
                },
            ],
            styles: {
                header: {
                    fontSize: 18,
                    bold: true,
                },
                subtitle: {
                    fontSize: 16,
                    bold: true,
                    color: 'gray',
                },
                titlePurple: {
                    color: '#8257e5',
                },
            },
            images: {
                Praelogo:
                    'data:image/png;base64,',
            },
        };

        const pdfDoc = printer.createPdfKitDocument(docDef);

        const chunks: any = [];

        pdfDoc.on('data', chunk => {
            chunks.push(chunk);
        });
        pdfDoc.on('end', () => {
            const result = Buffer.concat(chunks);
            return response.end(result);
        });

        // close the stream
        pdfDoc.end();
    } catch (err) {
        return response.status(400).json({ ok: true });
    }
});

equipamentsRouter.get('/qrcode/:id', async (request, response) => {
    try {
        const fonts = {
            Helvetica: {
                normal: 'Helvetica',
                bold: 'Helvetica-Bold',
                italics: 'Helvetica-Oblique',
                bolditalics: 'Helvetica-BoldOblique',
            },
        };
        const opts = {
            errorCorrectionLevel: 'H',
            quality: 1,
            margin: 1,
            color: {
                dark: '#8257e5',
                light: '#FFF',
            },
        };
        const { id } = request.params;
        const equipamentsRepository = getRepository(Equipament);

        const equipament = await equipamentsRepository.findOneOrFail(id);
        //@ts-ignore
        const QRCODEURL = await QRCode.toDataURL(
            `${process.env.BASE_URL_FRONT}/equipaments/details/${id}`,
            opts,
        );
        const printer = new PDFPrinter(fonts);
        const today = new Date();

        const docDef: TDocumentDefinitions = {
            defaultStyle: { font: 'Helvetica' },
            content: [
                {
                    image: 'Praelogo',
                    width: 150,
                    height: 42,
                    alignment: 'center',
                    margin: [0, 0, 0, 20],
                },
                {
                    image: 'QRCODE',
                    width: 250,
                    height: 250,
                    alignment: 'center',
                },
                {
                    text: equipament.name,
                    alignment: 'center',
                    style: 'header',
                    margin: [0, 20, 0, 0],
                },
                {
                    text: equipament.id,
                    alignment: 'center',
                    style: 'subtitle',
                    margin: [0, 5, 0, 0],
                },
                {
                    text: `${today.toLocaleString('pt-BR')}`,
                    alignment: 'center',
                    style: 'subtitle',
                    margin: [0, 20, 0, 0],
                },
            ],
            styles: {
                header: {
                    fontSize: 18,
                    bold: true,
                },
                subtitle: {
                    fontSize: 16,
                    bold: true,
                    color: 'gray',
                },
            },
            images: {
                Praelogo:
                    'data:image/png;base64,',
                QRCODE: QRCODEURL,
            },
        };

        const pdfDoc = printer.createPdfKitDocument(docDef);

        // pdfDoc.pipe(fs.createWriteStream('Report.pdf'));

        const chunks: any = [];

        pdfDoc.on('data', chunk => {
            chunks.push(chunk);
        });
        pdfDoc.on('end', () => {
            const result = Buffer.concat(chunks);
            return response.end(result);
        });

        // close the stream
        pdfDoc.end();
    } catch (err) {
        return response.status(400).json({ ok: true });
    }
});

export default equipamentsRouter;
